...
$q_sub = $db->query("SELECT a.sub_answer, a.score_array, d.sub_question, d.sub_score FROM assignmentsubmission a, courseassign e, course c, assignmentdetail d WHERE e.tutor=$tutor_id AND a.student_id=$student_id AND e.course=c.courseID AND a.assignment_id=$assignment_id AND d.id=$assignment_id");
