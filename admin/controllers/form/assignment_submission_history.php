...
// die(var_dump($_REQUEST['id']));
    ...
    $q_submission = $db->query("SELECT a.course_code, a.submission_date, a.format, a.status, a.answer_filepath, d.question, d.ass_filepath, d.score, d.sub_question FROM assignmentsubmission a, courseassign e, course c, assignmentdetail d WHERE a.student_id=$stud_id AND e.course=c.courseID AND a.assignment_id=$assignment_id AND d.id=$assignment_id ORDER BY a.course_code");
  ...
  $q_result = $db->query("SELECT * FROM assignmentresult WHERE student_id=$stud_id AND course_code='$course_code'");
