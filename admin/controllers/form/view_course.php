...
$query_course = $db->query("SELECT * FROM courseassign INNER JOIN course ON course.courseID=courseassign.course WHERE tutor=".$_REQUEST['id']."");
...
<div class="modal-content">
	<div class="modal-header">
				...
				<h4 class="modal-title">Course Assigned Information</h4>
			</div>
...
$(document).ready(function(){
	...
	$('#course_data').on('submit', function(){
		...
		$.ajax({
			url:"components/save_course_query.php",
			method:"POST",
			data:form_data,
	...
	$('.remove').click(function(){
				$assignID = $(this).attr('value');
			if(confirm("Are you sure you want to remove this?"))
			{
				$(this).parents('td').empty().append($result);
				$('.remove').attr('disabled', 'disabled');
				setTimeout(function(){
					window.location = 'components/delete_assign_course.php?assignID=' + $assignID;
				}, 1000);
			}
		});
});
